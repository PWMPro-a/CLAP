...
func TestCodexExecutorExecute_EmptyStreamCompletionOutputUsesOutputTextDelta(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_text.delta\",\"output_index\":0,\"content_index\":0,\"item_id\":\"msg_1\",\"delta\":\"o\"}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_text.delta\",\"output_index\":0,\"content_index\":0,\"item_id\":\"msg_1\",\"delta\":\"k\"}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_1\",\"object\":\"response\",\"created_at\":1775555723,\"status\":\"completed\",\"model\":\"gpt-5.4-mini-2026-03-17\",\"output\":[],\"usage\":{\"input_tokens\":8,\"output_tokens\":2,\"total_tokens\":10}}}\n\n"))
	}))
	defer server.Close()
...
func TestCodexExecutorExecute_EmptyExistingCompletionOutputUsesOutputTextDelta(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_text.delta\",\"output_index\":0,\"content_index\":0,\"item_id\":\"msg_1\",\"delta\":\"o\"}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_text.delta\",\"output_index\":0,\"content_index\":0,\"item_id\":\"msg_1\",\"delta\":\"k\"}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_1\",\"object\":\"response\",\"created_at\":1775555723,\"status\":\"completed\",\"model\":\"gpt-5.4-mini-2026-03-17\",\"output\":[{\"id\":\"msg_1\",\"type\":\"message\",\"role\":\"assistant\",\"status\":\"completed\",\"content\":[{\"type\":\"output_text\",\"text\":\"\"}]}],\"usage\":{\"input_tokens\":8,\"output_tokens\":2,\"total_tokens\":10}}}\n\n"))
	}))
	defer server.Close()
...
func TestCodexExecutorExecute_EmptyOutputItemDoneUsesOutputTextDelta(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_text.delta\",\"output_index\":0,\"content_index\":0,\"item_id\":\"msg_1\",\"delta\":\"o\"}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_text.delta\",\"output_index\":0,\"content_index\":0,\"item_id\":\"msg_1\",\"delta\":\"k\"}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.output_item.done\",\"item\":{\"id\":\"msg_1\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"\"}]},\"output_index\":0}\n"))
		_, _ = w.Write([]byte("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_1\",\"object\":\"response\",\"created_at\":1775555723,\"status\":\"completed\",\"model\":\"gpt-5.4-mini-2026-03-17\",\"output\":[],\"usage\":{\"input_tokens\":8,\"output_tokens\":2,\"total_tokens\":10}}}\n\n"))
	}))
	defer server.Close()
...
func TestPatchCodexCompletedOutputPreservesToolAndBackfillsText(t *testing.T) {
	outputText := &codexOutputTextAccumulator{}
	collectCodexOutputTextEvent([]byte(`{"type":"response.output_text.delta","output_index":1,"content_index":0,"item_id":"msg_1","delta":"answer"}`), outputText)

	completed := []byte(`{"type":"response.completed","response":{"id":"resp_1","output":[{"type":"function_call","id":"call_1","call_id":"call_1","name":"lookup","arguments":"{}"},{"id":"msg_1","type":"message","role":"assistant","content":[{"type":"output_text","text":""}]}]}}`)
	patched := patchCodexCompletedOutputWithText(completed, nil, nil, outputText)

	output := gjson.GetBytes(patched, "response.output").Array()
	if len(output) != 2 {
		t.Fatalf("output length = %d, want 2; payload=%s", len(output), patched)
	}
	if got := output[0].Get("type").String(); got != "function_call" {
		t.Fatalf("output.0.type = %q, want function_call; payload=%s", got, patched)
	}
	if got := output[1].Get("content.0.text").String(); got != "answer" {
		t.Fatalf("output.1.content.0.text = %q, want answer; payload=%s", got, patched)
	}
}

func TestPatchCodexCompletedOutputKeepsExistingVisibleText(t *testing.T) {
	outputText := &codexOutputTextAccumulator{}
	collectCodexOutputTextEvent([]byte(`{"type":"response.output_text.delta","output_index":0,"content_index":0,"item_id":"msg_1","delta":"fallback"}`), outputText)

	completed := []byte(`{"type":"response.completed","response":{"id":"resp_1","output":[{"id":"msg_1","type":"message","role":"assistant","content":[{"type":"output_text","text":"already"}]}]}}`)
	patched := patchCodexCompletedOutputWithText(completed, nil, nil, outputText)

	if !bytes.Equal(patched, completed) {
		t.Fatalf("non-empty completed output was unexpectedly patched: %s", patched)
	}
}
